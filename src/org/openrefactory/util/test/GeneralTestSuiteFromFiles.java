...
public abstract class GeneralTestSuiteFromFiles extends TestSuite
{
    ...
    private String getFullPath(String directorySuffix)
    {
        ...
        if (!rootDirectory.equals("") && !rootDirectory.endsWith(File.separator)) sb.append(File.separator); //$NON-NLS-1$ //$NON-NLS-2$

        sb.append(directorySuffix);
        if (!directorySuffix.endsWith(File.separator)) sb.append(File.separator); //$NON-NLS-1$
